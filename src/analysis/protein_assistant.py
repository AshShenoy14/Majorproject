...
GENERAL_FAQ = {
    ...
    "gat": "A **Graph Attention Network (GAT)** is a graph neural network that weights neighbors with an attention mechanism. TransGraph-PPI does **not** use a GAT: its graph model is **GraphSAGE**, which aggregates information from each protein's neighbors in the interaction graph (proteins are nodes, interactions are edges) and has no attention weights.",
    "ensemble": "An **ensemble model** combines predictions from multiple individual models to make more accurate final predictions. In TransGraph-PPI, we combine the ESM-MLP (sequence-based) and GraphSAGE (graph-based) predictions using XGBoost — achieving better accuracy than either model alone.",
        ...
        if any(kw in q for kw in ["what can you", "help me", "what do you", "your features", "capabilities"]):
            return {
                "response": "I can help you with many things! Here's what I know about:\n\n"
                           ...
                           "🤖 **TransGraph-PPI** — How our AI prediction system works (ESM-2, GraphSAGE, SHAP)\n\n"
