...
\noindent Protein–Protein Interactions (PPIs) are central to many biological processes, including cellular communication, metabolism, immune response, and disease progression. Understanding these interactions is essential for uncovering biological mechanisms and identifying potential drug targets. However, experimental methods for detecting PPIs are often expensive and time-consuming, making computational approaches an effective alternative.\\

\noindent This project, TransGraph-PPI, proposes a hybrid framework for predicting PPIs by combining deep learning and graph-based techniques. Traditional methods often struggle due to reliance on single models, handcrafted features, or unavailable structural data. Recent advances in protein language models and graph neural networks capture both sequence-level and network-level information, but are rarely integrated effectively.\\

\noindent To address this, the proposed method combines embeddings from the ESM-2 protein language model with a Graph Attention Network (GAT). ESM-2 generates meaningful representations of protein sequences, while GAT models interaction patterns within a protein network. These components are integrated using an ensemble strategy, where XGBoost acts as a meta-learner to enhance prediction accuracy.\\

\noindent The method begins with collecting validated PPI data from STRING and BioGRID. Protein sequences are converted into fixed-length embeddings using ESM-2, and a protein interaction graph is constructed for GAT-based link prediction. The outputs from both models are combined using soft voting and stacking techniques to produce final predictions.\\

\noindent To improve interpretability, SHAP analysis is used to identify feature contributions. Additionally, network centrality measures help identify key proteins that may act as potential drug targets, which are further validated using ChEMBL data.\\

\noindent Overall, TransGraph-PPI aims to outperform traditional approaches such as Random Forest by achieving better accuracy, F1-score, ROC-AUC, and PR-AUC. The framework improves both prediction performance and interpretability.\\

\noindent Future work includes extending the model to multi-species datasets, incorporating 3D structural information, and developing a web-based platform for visualization and drug target analysis.\\
