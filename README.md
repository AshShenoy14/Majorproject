...
> **Status: the table above predates the negative-sampling fix and is superseded.** It was produced on a dataset whose
> negatives included STRING interactions with score 1-899 (27.7% of negatives), with fold models trained for 5 epochs and
> a meta-learner that included a constant `bio_score` feature. The pipeline below was corrected; numbers must be
> regenerated by re-running it end to end (`assets/evaluation/final_test_metrics.json` is overwritten by `compare_models.py`).
> Even on clean negatives, a classifier that sees only node degree reaches ~71% test accuracy
> (`scripts/audit_measurements.py`), so a degree baseline should be reported next to any model result.
