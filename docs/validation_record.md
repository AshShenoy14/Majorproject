...
> **Superseded (pre-fix record).** The numbers and the 8-feature / `bio_score` description below were produced before the negative-sampling, OOF-leak, OOF-training-budget and `bio_score` fixes. The ensemble now uses 7 meta-features with a calibrated GraphSAGE probability. Regenerate results by re-running the pipeline (see README).
